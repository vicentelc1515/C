...
#include<stdlib.h>
#include<windows.h>
//#include<unistd.h> caso seja executado em Linu/Mac sleep(segundos)
#include <time.h>
...
Conta buscaContaPorNumero(int numero);
...
void criarConta(){
	...
	fgets(cliente.dataNasc, 20, stdin);
...
void efetuarTransferencia(){
	if(contador_contas > 0){
		...
		Conta conta_o = buscaContaPorNumero(numero_o);

		if(conta_o.numero == numero_o){
			printf("Informe o número da conta destino: \n");
			scanf("%d", &numero_d);

			Conta conta_d = buscaContaPorNumero(numero_d);
